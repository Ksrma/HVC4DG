...
of a set of 4-dimensional points. Please refer to the following paper 
for a description of this HVC4D-G algorithm: 

Jingda Deng, Qingfu Zhang, Jianyong Sun, and Hui Li, "An 
O(n^{3/2}logn) Exact Algorithm for Hypervolume Contributions 
in 4-D Space".

Compilation: g++ -O3 HVC4DG.cpp
Usage: HVC4DG <number of points> <input file> 
...
inline int containsBoundary(const double* cub, const double regUp[], const int split) {

	// condition only checked for split>0
	if (regUp[split] <= cub[split]){
...
inline void buildTree(YildizTreeNode *node, int split);
...
inline void buildTree(YildizTreeNode *node, int split) {
	...
	if (allPiles) {
		...
		if (count == node->partialCoverNumber) {
			newList = node->LinkedList[linkedListSize-1];
		} else {
			iterCount = 0;
			newList = new int[count];
			for (j=0; j<node->partialCoverNumber; j++) {
				if (treeProjection[node->LinkedList[linkedListSize-1][j]]) {
					newList[iterCount] = node->LinkedList[linkedListSize-1][j];
					iterCount++;
				}
			}
		}
	...
	do {
		for (i=0; i<count; i++) {
			int contained = containsBoundary(population[newList[newsplit - split][i]], node->upperbound, newsplit);
			if (contained == 1) {
				boundaries[boundSize] = newList[newsplit - split][i];
				boundSize++;
			} else if (contained == 0) {
				noBoundaries[noBoundSize] = newList[newsplit - split][i];
				noBoundSize++;
			}
		}
	...
	} while (middleIndex == -1 && newsplit < dimension-2);
	...
	leftChild->upperbound[newsplit] = population[middleIndex][newsplit];
	leftChild->LinkedList = vector<int*>(dimension - newsplit - 1);
	for (i=0; i<dimension - newsplit - 1; i++) {
		leftChild->LinkedList[i] = newList[i + newsplit - split];
	}
	for (i=0; i<count; i++) {
	...
	for (i=0; i<count; i++) {
		if (population[node->partialCoverIndex[i]][newsplit] > population[middleIndex][newsplit]) {	
	...
	if (rightChild->partialCoverNumber > 0) {
		...
		rightChild->lowerbound[newsplit] = population[middleIndex][newsplit];
		rightChild->isNewLinkedList = true;
		rightChild->LinkedList = vector<int*>(dimension - newsplit - 1);
		for (i=0; i<dimension - newsplit - 1; i++) {
			iterCount = 0;
		...
		buildTree(leftChild, newsplit);
		if (node->isNewLinkedList && !leftChild->isNewLinkedList) {
			for (i=0; i<linkedListSize; i++) {
				delete [] node->LinkedList[i];
			}
		}
		buildTree(rightChild, newsplit);
	} else {
...
int main(int  argc, char  *argv[]) {
	...
	/* check parameters */
	if (argc < 4)  {
		fprintf(stderr, "usage: HVC4DG <number of points> <input file> <reference point file> <outputfile(optional)>\n");
		exit(1);
	}
